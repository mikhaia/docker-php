...
<section class="blog-main">
    <div class="container">
        <div class="header">
            ...
            <h1 class="title page-name">Новости</h1></div>
                    ...
                    @foreach($items as $item)
                        <div class="posts-item" itemprop="blogPosts" itemscope itemtype="http://schema.org/BlogPosting">
                            <div class="posts-image">
                                <a href="{{ route('news.show', $item->url) }}"><img src="{{ $item->image }}" alt="{{ $item->title }}"></a>
                            </div>
                            <div class="posts-info">
                                <div class="date">{{ $item->date }}</div>
                                <div class="name"><a href="{{ route('news.show', $item->url) }}" itemprop="url">{{ $item->title }}</a></div>
